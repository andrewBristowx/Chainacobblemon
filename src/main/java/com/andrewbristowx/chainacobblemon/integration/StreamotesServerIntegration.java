...
public final class StreamotesServerIntegration {
    public static final String CHANNEL = "chainavt";
    ...
    public static void ensureOfficialChannel() {
        if (!FabricLoader.getInstance().isModLoaded("streamotes")) {
            Chainacobblemon.LOGGER.info("Streamotes not installed; emote channel synchronization skipped");
            return;
        }
        ...
        if (!SUPPORTED_VERSION.equals(version)) {
            Chainacobblemon.LOGGER.warn("Expected Streamotes {}, found {}. Automatic channel configuration skipped.", SUPPORTED_VERSION, version);
            return;
        }
        try {
            ...
            if (currentChannels instanceof List<?> existing) {
                for (Object entry : existing) {
                    if (entry instanceof String channel && !channel.isBlank() && !STREAMOTES_SAMPLE_CHANNELS.contains(channel.toLowerCase(Locale.ROOT))) channels.add(channel);
                }
            }
            if (channels.stream().noneMatch(CHANNEL::equalsIgnoreCase)) channels.add(CHANNEL);
            channelsField.set(model, channels);
            enable(modelClass, model, "twitchSubscriberEmotes"); enable(modelClass, model, "bttvChannelEmotes"); enable(modelClass, model, "ffzChannelEmotes"); enable(modelClass, model, "x7tvChannelEmotes");
            disable(modelClass, model, "twitchGlobalEmotes"); disable(modelClass, model, "bttvEmotes"); disable(modelClass, model, "ffzEmotes"); disable(modelClass, model, "x7tvEmotes"); disable(modelClass, model, "colorEmotes");
            requireExplicitCodes(modelClass, model); save.invoke(null);
        } catch (ReflectiveOperationException exception) { Chainacobblemon.LOGGER.warn("Could not configure Streamotes channel automatically", exception); }
    }
    private static void enable(Class<?> c, Object m, String f) throws ReflectiveOperationException { c.getField(f).setBoolean(m, true); }
    private static void disable(Class<?> c, Object m, String f) throws ReflectiveOperationException { c.getField(f).setBoolean(m, false); }
