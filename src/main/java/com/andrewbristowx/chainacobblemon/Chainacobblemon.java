...
import com.andrewbristowx.chainacobblemon.npc.ChainaNpcInteraction;
...
public final class Chainacobblemon implements ModInitializer {
    ...
    public static final String VERSION = "0.3.0-alpha.1+1.21.1";
    public static final Logger LOGGER = LoggerFactory.getLogger(MOD_ID);

    @Override
    public void onInitialize() {
        LOGGER.info("Iniciando Chainacobblemon {}", VERSION);
        ChainaRegistries.initialize();
        ChainaEquipment.initialize();
        StreamotesServerIntegration.ensureOfficialChannel();
        ChainaVisualAssetNetworking.initializeServer();
        GameplayNetworking.initializeServer();
        // Debe registrarse antes del callback genérico de GameplaySystems.
