...
public final class FishingMinigameService {
        ...
        try {
            ...
            if (ownedSpawn != null) {
                if (ownedSpawn.expiresAt > callbackNow) {
                    discardFishingEntity(entityValue, pokemon, ownedSpawn.playerId);
            ...
            if (active != null) {
                discardFishingEntity(entityValue, pokemon, active.playerId);
            ...
            discardFishingEntity(entityValue, pokemon, player.getUuid());
            suppressBattle(player.getUuid());
    ...
    private static void sendClientCleanup(UUID playerId, Pokemon pokemon, Entity entity) {
    ...
    private static void rememberPokemon(UUID playerId, Pokemon pokemon) {
        if (playerId == null || pokemon == null) return;
        Object value = reflected(pokemon, "getUuid");
        if (!(value instanceof UUID pokemonId)) return;
